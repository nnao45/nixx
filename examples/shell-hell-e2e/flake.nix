      ...
      in
      {
        packages = let
          ...
          e2eStrict = mkE2e "e2e-strict" ((n.mkTasks { name = "e2e-strict"; } {
            ...
            strict_on = n.task { strict = true; deps = [ "disable" ]; } (n.sh ''
            ...
            strict_off = n.sh ''
              # without strict, ''${UNDEF} doesn't fail (default uses +euo at function level
              # only if the outer script had it; but we test that strict=false
              # does NOT inject set -euo pipefail into the function body)
              UNDEF=''${UNDEF:-ok}
        ...
        devShells.default = pkgs.mkShell {
          ...
          shellHook = ''
            ...
            echo "  nix run .#e2e-strict -- all  strict mode"
